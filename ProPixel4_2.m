...
for k = 1:5
    ...
    [theta3{k}, phi3{k}] = naklonLok2(theta_0, phi_0, theta0{k}, phi0{k}, 0);
    [Theta3{k}, Phi3{k}] = naklonLok2(theta_0, phi_0, Theta0{k}, Phi0{k}, 0);
    
    % додаткові формули для локального повороту
    [theta4{k}, phi4{k}] = naklonLok2(theta_0, phi_0, theta0{k}, phi0{k}, 1);
    [Theta4{k}, Phi4{k}] = naklonLok2(theta_0, phi_0, Theta0{k}, Phi0{k}, 1);
    ...
    if k < 5
        ...
        % додаткові формули для локального повороту
        % спочатку по тангажу, а тоді по крену
        [Rtheta3{k}, Rphi3{k}] = naklonLok2(theta_0, phi_0, Rtheta0{k}, Rphi0{k}, 0);
        % спочатку по крену, а тоді по тангажу
        [Rtheta4{k}, Rphi4{k}] = naklonLok2(theta_0, phi_0, Rtheta0{k}, Rphi0{k}, 1);
    end
end
...
title('Проекція матриці пікселів (тангаж - крен) спрощена формула');
% plot(0,0, 'k^');
...
xlabel('Поперек польоту [м]');
ylabel('Вздовж польоту [м]');
title('Проекція матриці пікселів (крен - тангаж) спрощена формула');
...
xlabel('Поперек польоту [м]');
ylabel('Вздовж польоту [м]');
title('Проекція матриці пікселів (тангаж - крен) точніша формула');
...
xlabel('Поперек польоту [м]');
ylabel('Вздовж польоту [м]');
title('Проекція матриці пікселів (крен - тангаж) точніша формула');
